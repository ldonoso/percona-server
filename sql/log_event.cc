...
Slave_worker *Log_event::get_slave_worker(Relay_log_info *rli) {
  ...
  if ((is_s_event = starts_group()) || is_gtid_event(this) ||
    ...
    if (!rli->curr_group_seen_gtid && !rli->curr_group_seen_begin) {
      ...
      assert(rli->curr_group_seen_gtid);
...
int Log_event::apply_event(Relay_log_info *rli) {
  ...
  assert(!(rli->curr_group_seen_begin && ends_group()) ||
         ...
         */
         (rli->curr_group_seen_begin && rli->curr_group_seen_gtid &&
          ends_group()) ||
         is_mts_parallel_type_dependency(rli) || is_mts_db_partitioned(rli) ||
...
int Rows_log_event::unpack_current_row(const Relay_log_info *const rli,
  ...
  if (type == Log_event_type::WRITE_ROWS_EVENT ||
    ...
    assert(type == Log_event_type::UPDATE_ROWS_EVENT ||
...
static bool is_any_column_signaled_for_table(TABLE *table, table_def *tabledef,
  ...
  if (tabledef->use_column_names(table)) {
    assert(tabledef->size() == cols2->n_bits);
...
void Rows_log_event::set_readwriteset_from_col_names(TABLE *table,
  ...
  assert(tabledef->have_column_names());
  ...
  AFTER_MAIN_EXEC_ROW_LOOP:
    ...
    */
    ...
    if (m_force_binlog_idempotent && !error) {
      ...
      assert(rbr_exec_mode == RBR_EXEC_MODE_IDEMPOTENT);
      assert(thd->is_enabled_idempotent_recovery());
...
bool Table_map_log_event::init_fb_format_sign_bits() {
  if (m_sign_bits_size * 8 >= m_table->s->fields) {
    ...
    assert(m_sign_bits_size <= m_table->s->fields);
    for (unsigned int i = 0; i < m_table->s->fields; ++i) {
      if (m_table->field[i]->is_flag_set(UNSIGNED_FLAG))
...
int Rows_query_log_event::do_apply_event(Relay_log_info const *rli) {
  ...
  if (has_trx_meta_data()) {
    ...
    assert(strstr(m_rows_query, "*/") != nullptr);
...
void Query_log_event::prepare_dep(Relay_log_info *rli,
  ...
  if (starts_group()) {
    ...
    assert(submode->prev_event != nullptr);
    assert(ev->begin_event() != nullptr);
    assert(!ev->begin_event()->whole_group_scheduled);
...
void Xid_log_event::prepare_dep(MY_ATTRIBUTE((unused)) Relay_log_info *rli,
  ...
  assert(is_mts_parallel_type_dependency(rli));
  assert(static_cast<Mts_submode_dependency *>(rli->current_mts_submode)
             ->prev_event != nullptr);
  assert(ev->begin_event() != nullptr);
  assert(!ev->begin_event()->whole_group_scheduled);
  ...
  for (i = 0, curr_row = m_rows_buf; curr_row != m_rows_end;
    ...
    for (const auto &key_info : key_infos) {
      ...
      for (uint j = 0; j < key_info->user_defined_key_parts; j++) {
        curr_key.key_length +=
            std::min((ulong)key_info->key_part[j].field->key_length(),
...
bool Rows_log_event::get_keys(Relay_log_info *rli,
  ...
  assert(rli->mts_dependency_replication == DEP_RPL_STATEMENT);
  assert(thd->open_tables == nullptr);
  assert(m_key_info == nullptr);
  assert(m_table == nullptr);
  assert(m_curr_row_end == nullptr);
  ...
  if (unlikely(!get_table_ref(rli, &memory, &table_list))) {
    assert(rli->tables_to_lock == nullptr);
  ...
  assert(table_list->m_tabledef_valid);
...
bool Rows_log_event::get_table_ref(Relay_log_info *rli, void **memory,
  ...
  assert(submode->table_map_events.count(get_table_id()));
  assert(memory != nullptr);
