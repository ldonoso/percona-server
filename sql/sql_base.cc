...
void return_table_to_cache(THD *thd, TABLE_LIST *table_list) {
  ...
  if (table_list->table != nullptr) {
    assert(thd->open_tables == table_list->table);
    assert(thd->open_tables->next == nullptr);
    assert(!table_list->table->s->has_old_version());
...
Field *find_field_in_table_sef(TABLE *table, const char *name) {
  ...
  for (uint i = 0; i < table->s->fields; ++i)
    if (table->field[i] &&
        !my_strcasecmp(system_charset_info, table->field[i]->field_name, name))
