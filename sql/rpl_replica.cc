...
extern "C" void *handle_slave_sql(void *arg) {
  ...
  my_thread_init();
  {
    ...
    if (replica_preserve_commit_order && !rli->is_parallel_exec() &&
...
int connect_to_master(THD *thd, MYSQL *mysql, Master_info *mi, bool reconnect,
...

  {
    char *algorithm;
    char buf[64];
    if (opt_replica_compressed_protocol) {
#ifndef NDEBUG
      int ret =
#endif /* NDEBUG */
          /*
          ...
          */
          snprintf(buf, sizeof(buf), "%s,%s",
                   mysql_compression_lib_names[opt_replica_compression_lib],
                   COMPRESSION_ALGORITHM_UNCOMPRESSED);
      assert(ret >= 0 && static_cast<unsigned int>(ret) < sizeof(buf));
      algorithm = buf;
    } else {
...
static int check_slave_sql_config_conflict(const Relay_log_info *rli) {
  ...
  const auto replica_preserve_commit_order = get_slave_preserve_commit_order();
  if (replica_preserve_commit_order && replica_parallel_workers > 0) {
